...
exports.handler = async (event) => {
  try {
    const action = event.path.replace(/^\/(?:api|\.netlify\/functions\/api)\/?/, "").replace(/\/$/, "");
    ...
    const user = getSessionUser(state, event.headers.authorization);
    ...
    if (action === "auth/logout") return logout(state, event.headers.authorization);
